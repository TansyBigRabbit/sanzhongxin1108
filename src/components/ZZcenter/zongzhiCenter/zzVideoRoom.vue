...
<div class="breadcrumb">
  ...
  <el-button v-if="pageToData.pageToRole=='comprehensiveTreatmentCreate'" type="primary" style="width: 150px" @click="quitRoom(1)">结束房间</el-button>
...
<script> 
       ...
       initWebRTC: function(did,roleType) { 
         //name=ILiveSDK.loginInfo.identifier; 
		      var message = {
		      	       ...
		      	       //房间名称
		      	       comprehensiveTreatmentName:that.pageToData.pageToRoom,
		      	       room:that.pageToData.room,
		      	       //面谈类型
		      	       comprehensiveTreatmentType:"XXX",
		      	       //
		      	       initiatorIdCard:window.localStorage.getItem("idCard"),
                       receiveDepart:that.departId,
    ...
    ws.onmessage = function(message) {
       ...
       if(that.pageToData.pageToRole=="comprehensiveTreatmentCreate"){
		    	console.log("创建者创建面谈房间");
		    	app.createRoom();
		    }else if(that.pageToData.pageToRole=="comprehensiveTreatmentJoin"){
...
getRoomList: function(opts, succ, err) {
  ...
  if(type==1){
  	//结束房间
    param = "comprehensiveTreatmentFinish";
  }else if(type==2){
  	//退出房间
  	param = "comprehensiveTreatmentLeave";
  }
  	var self = this;
  	console.log("关闭...")
